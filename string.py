...
class Ball:
    def __init__(self):  # Class for massive points
        self.x = 0
        self.y = 0
        self.Vx = 0
        self.Vy = 0
...
def start_from(x_0, y_0):
    for i in range(0, len(ball)):
        ball[i].x = (i + 1) * x_0  # initial x coordinates of points
        ball[i].y = y_0  # initial y coordinates of points


def move_ball():  # moves the points
    for i in range(0, len(ball)):
...
def acceleration(parameter, l_0, u):
    """
    Contains the laws of movement
    :param l_0: distance between points in equilibrium
    :param parameter: = stiffness / mass
    :param u: velocity of the point being pushed
    """
    ...
    for i in range(0, len(ball)):
        ...
        if i < main_ball_num and i != 0:
            ...
            # ball[i].Vx += ball[i].Ax
            # ball[i].Vy += ball[i].Ay
        if i == main_ball_num:  # middle point
            ball[i].Vy = u
...
def rendering():  # rendering the string
    ...
    canv.create_oval(ball[len(ball) - 1].x, ball[len(ball) - 1].y - 2.5, ball[len(ball) - 1].x + 5,
                     ball[len(ball) - 1].y + 2.5)
